...
def change_ncls_admin(request):
    if request.method == 'GET':
        ...
        if request.user.is_authenticated and request.user.is_superuser:
            fields = {}
            fields['Nckh'] = request.FILES.get('Nckh')
            fields['Kqls'] = request.FILES.get('Kqls')
            try:
                obj = Edit_ncls.objects.get(Count=1)
                ...
                fields['Count'] = 1
                Edit_ncls.objects.create(**fields)
            return redirect('change_ncls_admin')
        else:
            return JsonResponse({'success': False, 'message': 'Bạn chưa được cấp quyền do tài khoản chưa đăng nhập hoặc tài khoản không có quyền truy cập'})
