...
def change_kqls_admin(request):
    if request.method == 'GET':
        context = {}
        context['domain'] = settings.DOMAIN
        try:
            context['obj'] = Edit_kqls.objects.get(Count=1)
        except:
            context['obj'] = {}
        # print('context:',context)
        if request.user.is_authenticated and request.user.is_superuser:
            return render(request, 'sleekweb/admin/change_kqls_admin.html', context, status=200)
        else:
        ...
        if request.user.is_authenticated and request.user.is_superuser:
            ...
            try:
                obj = Edit_kqls.objects.get(Count=1)
                ...
                fields['Count'] = 1
                Edit_kqls.objects.create(**fields)
            return redirect('change_kqls_admin')
        else:
            return JsonResponse({'success': False, 'message': 'Bạn chưa được cấp quyền do tài khoản chưa đăng nhập hoặc tài khoản không có quyền truy cập'})
