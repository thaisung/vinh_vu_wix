...
def change_tnsm_admin(request):
    if request.method == 'GET':
        context = {}
        context['domain'] = settings.DOMAIN
        try:
            context['obj'] = Edit_tnsm.objects.get(Count=1)
        except:
            context['obj'] = {}
        ...
        if request.user.is_authenticated and request.user.is_superuser:
            ...
            try:
                obj = Edit_tnsm.objects.get(Count=1)
                ...
                fields['Count'] = 1
                Edit_tnsm.objects.create(**fields)
            return redirect('change_tnsm_admin')
        else:
            return JsonResponse({'success': False, 'message': 'Bạn chưa được cấp quyền do tài khoản chưa đăng nhập hoặc tài khoản không có quyền truy cập'})

def change_tnsm1_admin(request):
    if request.method == 'GET':
        if request.user.is_authenticated and request.user.is_superuser:
            fields = {}
            ...
            try:
                if fields['id']:
                    obj = Edit_tnsm1.objects.get(id=fields['id'])
                    obj.delete()
            except:
                print('Not obj')
            return redirect('change_tnsm_admin')
    if request.method == 'POST':
        if request.user.is_authenticated and request.user.is_superuser:
            ...
            if fields['id']:
                ...
                Edit_tnsm1.objects.create(**fields)
            return redirect('change_tnsm_admin')
        else:
            return JsonResponse({'success': False, 'message': 'Bạn chưa được cấp quyền do tài khoản chưa đăng nhập hoặc tài khoản không có quyền truy cập'})

def change_tnsm1_order_admin(request):    
    if request.method == 'POST':
        context = {}
        id = request.POST.get('id')
        Order = request.POST.get('Order')
        try:
            obj = Edit_tnsm1.objects.get(pk=id)
            obj.Order = Order
            obj.save()
        except:
            return redirect('change_tnsm_admin')
